...
def summarize(clusters, exact, k, models, maxNs, slo=True):
    for i in range(k):  # for each cluster
        ...
        print(f"maxN:\n\t" + " \t".join([str(maxN) for maxN in maxNs]))
...
def cluster(folder, ks=None, slo=True, logscale=None):
    for logarithmic_scale in logscale:
        ...
        parameters = [(int(p[0]), int(p[-1]), p[1:-1]) for p in [par.split("_") for par in
                                                                 open(f'collectedData/data{folder}/'
        ...
        data, eigval, eigvec, meandata = pca(data0)
        bins = np.linspace(-1 if logarithmic_scale else 0, 8 if logarithmic_scale else 1e8, len(eigval))

        plt.figure(figsize=(17, 11))
        plt.suptitle(f"{'Log' if logarithmic_scale else 'Lin'}, maxN <= {folder}")
        ...
        for k in ks:  # compute k clusters, means of clusters
            ...
            plt.figure(figsize=(17, 11))
            plt.suptitle(f"{'Log' if logarithmic_scale else 'Lin'}, maxN <= {folder}, k = {k}")
            plt.subplot(221)
            ...
            plt.title("Preživetvena funkcija povprečij gruč" if slo else "Survival function of cluster means")
            plt.legend(loc="best")
            ...
            for i in range(k):
                ...
                p2 = points[np.argmin(np.sum((-(p1[:2] - m) / 2 + (p1[:2] - m)[::-1] * np.sqrt(3) / 2 -
                                              points[:, :2]) ** 2, 1)), :]
                p3 = points[np.argmin(np.sum((-(p1[:2] - m) / 2 - (p1[:2] - m)[::-1] * np.sqrt(3) / 2 -
                                              points[:, :2]) ** 2, 1)), :]
            ...
            plt.xlabel(f"Expected civilization longevity in {'log(years)' if logarithmic_scale else 'years'}" if slo
                       else f"Expected civilization longevity in {'log(years)' if logarithmic_scale else 'years'}")
            ...
            plt.subplot(224)
            # draw means of clusters
            plt.plot(bins, np.mean(means, 1), label="Povprečno povprečje" if slo else "Mean mean")
            plt.plot(bins, np.mean(data0, 0)/np.sum(np.mean(data0, 0)), label="Splošno povprečje" if slo else "Mean of all")
            plt.xlabel(f"{'log(L)' if logarithmic_scale else 'L'}")
            plt.ylabel("Povprečna gostota" if slo else "Mean density")
            plt.title("Povprečen histogram" if slo else "Mean histogram")
            plt.legend(loc="best")
                ...
                for maxN in maxNs:  # set size of marker to log10(maxN)
                    ...
                    if maxN == maxNs[-1]:    # add label to legend only oce per model
                        ax.plot(points[:, 0],  # color by maxN
                                points[:, 1],
                                points[:, 2],
                                marker='o', markersize=4, linewidth=0, color=colors[model - 1],
                                label=f"Model {model}")
                    else:
                        ax.plot(points[:, 0],  # color by maxN
                                points[:, 1],
